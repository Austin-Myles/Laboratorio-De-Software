...
# Script de compilación automática para NicoustinRobot (Arquitectura de Clases Internas)
# Uso: ./build.sh [ROBOCODE_LIBS_PATH]
# 
# Compila todos los archivos Java con estrategias como clases internas
# Si no se especifica ROBOCODE_LIBS_PATH, busca robocode.jar en ubicaciones comunes

echo "🔨 Compilando NicoustinRobot (Arquitectura Interna)..."
...
if [ $? -eq 0 ]; then
    ...
    # Copiar clases internas generadas por el compilador
    cp estrategas/*\$*.class build/estrategas/ 2>/dev/null || true
